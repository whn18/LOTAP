function [predict_data, out] = LOTAP(data, opts)
...
% * |opts.itermax|: maximum iter counts
% * |opts.core_size|: core size of data
% * |opts.isReal|: Whether the input data is real or not. If it is real, it
% is 1, otherwise it is 0. Default=0.
...
for iter = 1: opts.itermax
    ...
    f_now2 = 0;     % Record the gap between S and S_hat
    f_now1 = 0;     % Record the gap between X_t and U*S_t*V^H
    for t = 1: T
    ...
    % if opts.isclearG=1, then clear the non-diagonal elements of fft_S
    if opts.isclearG
        for t=1: T
            for i = 1: n3
